...
const API_AUDIT_ENDPOINT = '/api/audit';
const LOCAL_STATE_KEY = 'sistema-cgpe:state:v1';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const hydrate = (state, sourceLabel) => {
      if (!state || !Array.isArray(state.users) || !Array.isArray(state.tasks) || !state.systemConfig) return false;

      const safeUsers = state.users.length > 0 ? state.users : INITIAL_USERS;
      const activeUser = safeUsers.find((u) => u.id === currentUser?.id) || safeUsers[0];

      setUsers(safeUsers);
      setCurrentUser(activeUser);
      setTasks(state.tasks);
      setSystemConfig(state.systemConfig);
      setTempConfig(state.systemConfig);
      setTempUsers(safeUsers);
      setHistory([{ timestamp: Date.now(), data: state.tasks }]);
      setHistoryIndex(0);
    ...
    const loadState = async () => {
      try {
        ...
        if (!cancelled && hydrate(serverState, 'api')) {
          setIsHydrated(true);
          return;
        }
      } catch (_) {
      ...
      try {
        const localRaw = localStorage.getItem(LOCAL_STATE_KEY);
        if (!localRaw) throw new Error('Sem estado local.');
        const localState = JSON.parse(localRaw);
        if (!cancelled && hydrate(localState, 'local')) {
          setIsHydrated(true);
          return;
        }
      } catch (_) {
        // fallback seed abaixo
      }
  ...
  useEffect(() => {
    ...
    if (tasks.some((tk) => tk.is_running)) return;

    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    saveTimerRef.current = setTimeout(async () => {
      const snapshot = { users, tasks, systemConfig };
      setIsSaving(true);
      ...
      try {
        await fetch(API_STATE_ENDPOINT, {
          ...
          headers: {
            ...
            'x-user-name': currentUser?.name || 'Usuário',
          },
          body: JSON.stringify(snapshot),
        });
      } catch (_) {
        // API indisponivel, estado local ja salvo
      }
  ...
  }, [users, tasks, systemConfig, currentUser, isHydrated]);

  useEffect(() => {
    if (!isHydrated || currentUser.role !== 'admin' || currentView !== 'auditoria') return;

    const loadAudit = async () => {
      setAuditLoading(true);
      try {
        const res = await fetch(`${API_AUDIT_ENDPOINT}?limit=120`);
        if (!res.ok) throw new Error('Falha ao carregar auditoria.');
        const payload = await res.json();
        setAuditTrail(Array.isArray(payload.events) ? payload.events : []);
      } catch (_) {
        setAuditTrail([]);
      } finally {
    ...
    loadAudit();
  }, [isHydrated, currentUser, currentView]);
  ...
  const handleExportJson = () => {
    const payload = { exportedAt: new Date().toISOString(), users, tasks, systemConfig };
    downloadTextFile(`cgpe-dados-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(payload, null, 2), 'application/json');
    showToast('Exportação JSON concluída.');
  };

  const handleExportCsv = () => {
    const headers = ['id', 'entrega', 'etapas', 'responsavel', 'status', 'inicio', 'fim', 'peso2', 'colaborador', 'tempo_real_segundos'];
    const lines = tasks.map((t) => {
      const responsavel = users.find((u) => u.id === t.assigned_to)?.name || 'Nao definido';
      const cols = [t.id, t.title, t.etapas, responsavel, t.status, t.data_criacao, t.data_fim, t.peso2, t.has_collaborator ? 'sim' : 'nao', t.actual_seconds || 0];
      return cols.map((c) => `"${String(c ?? '').replaceAll('"', '""')}"`).join(';');
    });
    const csv = [headers.join(';'), ...lines].join('\n');
    downloadTextFile(`cgpe-atividades-${new Date().toISOString().slice(0, 10)}.csv`, csv, 'text/csv');
  ...
  const handleSaveConfig = (e) => {
    ...
    if (upd) { setCurrentUser(upd); if (upd.role!=='admin' && (currentView==='configuracoes' || currentView==='auditoria')) setCurrentView('tabela_dinamica'); }
  ...
  const tasksWithCalculations = useMemo(() => {
    return tasks.map(task => {
      ...
      monthTasks.forEach(t => {
        const processWeight = getTaskProcessWeight(t) * (t.complexidade || 1);
        const key = `${getTaskProcess(t)}|${t.title}`;
        if(!eMap[key]) { eMap[key] = { weight: processWeight, count: 0 }; }
        eMap[key].count++;
      });
      ...
      const myKey = `${getTaskProcess(task)}|${task.title}`;
      const myED = eMap[myKey] || { weight: getTaskProcessWeight(task) * (task.complexidade || 1), count: 1 };
  ...
  const COLUMN_DEFS = {
    ...
    processo:     { label: 'Processo',    minWidth: 180 },
    pesoProcesso: { label: 'Peso Processo', width: 110, center: true, highlight: 'indigo' },
    entrega:      { label: 'Entrega',      minWidth: 260 },
    peso2:        { label: 'Peso Entrega', width: 90,  center: true, highlight: 'emerald' },
    complexidade: { label: 'Complexidade', width: 100, center: true, highlight: 'amber' },
  ...
  const renderCell = (colId, task) => {
    ...
    switch(colId) {
      ...
      case 'processo':
        {
          const processName = getTaskProcess(task);
          return (
            <td key={colId} className="py-2.5 px-3">
              {isAdmin
                ? <>
                    <input list="processo-options" value={processName} onChange={e=>handleTaskFieldChange(task.id,'processo',e.target.value)} className={`${inputCls} text-xs font-semibold`} />
                    <datalist id="processo-options">
                      {uniqueProcesses.map(p=><option key={p} value={p}/>) }
                    </datalist>
                  </>
                : <span className="font-semibold text-[#0B2461] text-xs break-words whitespace-normal" title={processName}>{processName}</span>}
            </td>
          );
        }
      case 'pesoProcesso':
        return (
          <td key={colId} className="py-2.5 px-3 text-center" style={{background:'rgba(99,102,241,0.08)'}}>
            <select value={task.pesoProcesso ?? getTaskProcessWeight(task)} onChange={e=>handleTaskFieldChange(task.id,'pesoProcesso',parseInt(e.target.value)||30)} disabled={!isMyTask||task.status==='Concluído'} className={`${selectCls} font-bold w-full`}>
              <option value={10}>10 pts</option>
              <option value={30}>30 pts</option>
              <option value={60}>60 pts</option>
            </select>
      ...
      case 'entrega':
        return (
          <td key={colId} className="py-2.5 px-3">
            {isAdmin
              ? <textarea rows={2} value={task.title} onChange={e=>handleTitleChange(task.id,e.target.value)} className={`${inputCls} min-h-[52px] text-xs`} />
              : <span className="text-xs text-gray-700 break-words whitespace-normal" title={task.title}>{task.title}</span>}
          </td>
        );
      case 'peso2':
        return (
      ...
      case 'complexidade':
        return (
          <td key={colId} className="py-2.5 px-3 text-center" style={{background:'rgba(245,158,11,0.08)'}}>
            <select value={task.complexidade||1} onChange={e=>handleTaskFieldChange(task.id,'complexidade',parseInt(e.target.value)||1)} disabled={!isMyTask||task.status==='Concluído'} className={`${selectCls} font-bold w-full`}>
      ...
      case 'atividade':
        return <td key={colId} className="py-2.5 px-3"><span className="text-xs text-gray-700 break-words whitespace-normal" title={task.etapas}>{task.etapas}</span></td>;
      case 'cronograma':
        return (
          <td key={colId} className="py-2.5 px-3">
        ...
        <aside className="sidebar-grd relative w-60 flex flex-col shadow-2xl z-20 shrink-0">
          ...
          <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
            ...
            {currentUser?.role==='admin' && (
              <>
                ...
                <NavItem icon={<Activity size={16}/>} label="Auditoria" active={currentView==='auditoria'} onClick={()=>setCurrentView('auditoria')}/>
          ...
          <div className="px-3 pb-4 pt-2 flex-shrink-0 border-t border-white/10">
            ...
            <select value={currentUser?.id}
              onChange={e=>{const u=users.find(u=>u.id===parseInt(e.target.value));setCurrentUser(u);if(u.role!=='admin'&&(currentView==='configuracoes'||currentView==='auditoria'))setCurrentView('tabela_dinamica');}}
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <header className="bg-white border-b border-gray-200/80 px-6 py-4 flex justify-between items-center flex-shrink-0 shadow-sm">
            <h1 className="font-sora font-bold text-[#0B2461] text-base">
              ...
              {currentView==='auditoria'       && 'Trilha de Auditoria'}
            ...
            <div className="flex items-center gap-3">
              <div className="text-right mr-1">
                ...
                <p className="text-[10px] text-gray-400 text-right">
                  {isSaving ? 'Sincronizando...' : `Fonte: ${dataSource.toUpperCase()}${lastSavedAt ? ` · ${lastSavedAt.toLocaleTimeString('pt-BR')}` : ''}`}
                </p>
              </div>
              ...
              <div className="flex flex-col gap-4 h-full">
                ...
                <div className="flex items-center justify-between flex-wrap gap-3 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleExportJson} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 shadow-sm transition-colors"><Download size={14}/> Exportar JSON</button>
                    <button onClick={handleExportCsv} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 shadow-sm transition-colors"><FileText size={14}/> Exportar CSV</button>
            ...
            {currentView==='dashboard' && (
              <div className="max-w-4xl mx-auto flex flex-col gap-6">
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">Peso Entrega por entrega + Monitoramentos × 60 · Ajustado por modalidade</p>
                    </div>
                  </div>
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-900 leading-relaxed space-y-1">
                  <p><strong>Metodologia — Carga de Trabalho:</strong> Soma dos pontos de Peso Entrega de cada entrega + (Qtd. Monitoramentos × 60 pts por servidor). O fator de modalidade é aplicado como multiplicador da meta esperada, usando os parâmetros definidos em Configurações Globais.</p>
                  <p><strong>Regra automática:</strong> Entregas de "Projetos de Melhoria" com colaborador marcado têm o Peso Entrega reduzido de 60 para 30 pontos no cálculo.</p>
                </div>
              </div>
            )}

            {/* ═══ AUDITORIA ═══ */}
            {currentView==='auditoria' && currentUser.role==='admin' && (
              <div className="max-w-5xl mx-auto flex flex-col gap-4">
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center justify-between">
                  <div>
                    <h2 className="font-sora font-bold text-[#0B2461] text-lg">Eventos de Atualização</h2>
                    <p className="text-xs text-gray-500 mt-1">Registro das alterações persistidas via API.</p>
                  </div>
                  <button onClick={()=>setCurrentView('tabela_dinamica')} className="px-3 py-2 text-xs font-semibold bg-blue-50 text-[#0B2461] border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors">
                    Voltar para tabela
                  </button>
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        <th className="text-left px-4 py-3 text-[10px] font-sora font-semibold uppercase tracking-widest text-gray-400">Data/Hora</th>
                        <th className="text-left px-4 py-3 text-[10px] font-sora font-semibold uppercase tracking-widest text-gray-400">Ator</th>
                        <th className="text-left px-4 py-3 text-[10px] font-sora font-semibold uppercase tracking-widest text-gray-400">Ação</th>
                        <th className="text-left px-4 py-3 text-[10px] font-sora font-semibold uppercase tracking-widest text-gray-400">Detalhes</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {auditLoading && (
                        <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">Carregando trilha...</td></tr>
                      )}
                      {!auditLoading && auditTrail.length===0 && (
                        <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">Nenhum evento encontrado.</td></tr>
                      )}
                      {!auditLoading && auditTrail.map((evt) => (
                        <tr key={evt.id} className="hover:bg-gray-50/60">
                          <td className="px-4 py-3 text-xs text-gray-600">{new Date(evt.timestamp).toLocaleString('pt-BR')}</td>
                          <td className="px-4 py-3 text-xs font-semibold text-gray-700">{evt.actorName}</td>
                          <td className="px-4 py-3 text-xs text-[#0B2461] font-semibold">{evt.action}</td>
                          <td className="px-4 py-3 text-xs text-gray-500">{evt.details}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl modal-anim overflow-hidden">
              ...
              <form onSubmit={handleAddTask} className="p-6 space-y-4">
                <div>
                  ...
                  <input list="processo-options" value={newTask.processo} onChange={e=>setNewTask({...newTask,processo:e.target.value})} required className={`${inputCls} font-semibold text-[#0B2461]`} placeholder="Digite ou selecione o processo…" />
                  <datalist id="processo-options">
                    {uniqueProcesses.map(p=><option key={p} value={p}/>) }
                  </datalist>
                </div>
                <div>
                  ...
                  <textarea required rows={3} value={newTask.title} onChange={e=>setNewTask({...newTask,title:e.target.value})} className={inputCls} placeholder="Descreva a entrega…"/>
                </div>
